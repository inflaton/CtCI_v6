...
public class Question {
  ...
  public static Edge[] createEdges(Piece[][] puzzle, int column, int row) {
    ...
    /* Get bottom edge */
    Edge bottom =
        row == puzzle.length - 1 ? new Edge(Shape.FLAT, key + "v|e") : createRandomEdge(key + "v");
    ...
    for (int r = 0; r < solution.length; r++) {
      for (int c = 0; c < solution[r].length; c++) {
        ...
        if (c < solution[r].length - 1) {
          ...
          if (!right
              .getEdgeWithOrientation(Orientation.LEFT)
        ...
        if (r < solution.length - 1) {
          ...
          if (!bottom
              .getEdgeWithOrientation(Orientation.TOP)
