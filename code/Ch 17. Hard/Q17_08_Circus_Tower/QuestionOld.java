...
public class QuestionOld {
  ...
  private static void longestIncreasingSubsequence(
      ArrayList<HtWt> array, ArrayList<ArrayList<HtWt>> solutions, int current_index) {
    ...
    for (int i = 0; i < current_index; i++) {
      if (array.get(i).isBefore(current_element)) { // If current_element is bigger than list tail
        best_sequence =
            seqWithMaxLength(best_sequence, solutions.get(i)); // Set best_sequence to our new max
      }
    }
